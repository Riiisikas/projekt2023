...
const int arraySize = 3; //tõenäoliselt muutub 3-ks
const int servoPin = 6;
unsigned long feedingTimes[arraySize] = {"a", "a", "a"}; //algsed suurused 0-id
int arrayIndex = 0;
...
void printWEB(WiFiClient client) {
    if (client) {
            ...
            if (client.available()) {
                ...
                if (c == '\n' && referer.startsWith("GET /submit")) {
                    ...
                    if (hourIndex != -1 && minuteIndex != -1 && feedingTimeIndex != -1) {
                        ...
                        int selectedFeedingTimeIndex = feedingTimeStr.toInt() - 1;
        ...
        client.println("</head>");
        client.println("<body>");
        client.println("<h2>Kausi massi arvutamine:</h2>");
        client.println("<form method='GET' action='/tare'>");
        client.println("<input type='submit' value='Alusta'>");
        client.println("</form>");
        if (_tare) {
        ...
        client.println("Vali kellaaja koht (1-3): <input type='number' name='feedingTime' min='1' max='3' /><br><br>");
        client.println("<input type='submit' value='OK'>");
        client.println("</form>");
...
void createFeedingTime(int hours, int minutes, int index) {
  ...
  feedingTimes[index-1] = timeInSeconds;
}
...
void calibrate(){
  ...
  float newCalibrationValue = LoadCell.getNewCalibration(tareValue);
  LoadCell.update();
